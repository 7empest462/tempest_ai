...
pub async fn run_daemon() {
    println!(
        "[{}] 🛡️ Tempest AI Daemon Initializing in Headless Mode...",
        Local::now().format("%H:%M:%S")
    );
    println!(
        "The agent is now actively patrolling your local memory limits and network routing topologies unconditionally in the background.\n"
    );
    ...
    loop {
        ...
        if used_ram >= memory_threshold_mb {
            println!(
                "[{}] ⚠️ CRITICAL RAM SPIKE: {} / {} MB",
                Local::now().format("%H:%M:%S"),
                used_ram,
                total_ram
            );
            let message = format!(
                "CRITICAL RAM SPIKE: {} MB currently mapped. System integrity in danger.",
                used_ram
            );
...
<dict>
    ...
    <true/>
        ...
        if let Err(e) = std::fs::write(plist_path, plist_content) {
            println!(
                "❌ Failed to write plist. Make sure to run with 'sudo tempest_ai --install-daemon'. Error: {}",
                e
            );
            return;
        }
...
WantedBy=multi-user.target"#;
        ...
        if let Err(e) = std::fs::write(service_path, service_content) {
            println!(
                "❌ Failed to write service file. Make sure to run with 'sudo tempest_ai --install-daemon'. Error: {}",
                e
            );
            return;
        }
