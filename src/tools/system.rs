...
impl AgentTool for SystemdManagerTool {
    ...
    fn description(&self) -> &'static str {
        "Controls systemd services (start, stop, restart, status, list) on Linux."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<SystemdManagerArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        let typed_args: SystemdManagerArgs =
            serde_json::from_value(args.clone()).into_diagnostic()?;
        let _ = &typed_args; // Sink to avoid unused warning on non-linux
...
impl AgentTool for CurrentProcessTool {
    ...
    fn description(&self) -> &'static str {
        "Returns technical details about the agent's own process state (RAM, uptime)."
    }
    fn tool_info(&self) -> ToolInfo {
        let mut settings = schemars::generate::SchemaSettings::draft07();
        settings.inline_subschemas = true;
