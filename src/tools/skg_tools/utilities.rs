    ...
    match action.as_str() {
        "write" => {
            let text = content.ok_or_else(|| {
                ToolError::ExecutionFailed("Missing 'content' for clipboard write".to_string())
            })?;
            let mut clip = arboard::Clipboard::new().map_err(|e| {
                ToolError::ExecutionFailed(format!("Failed to access clipboard: {}", e))
            })?;
            clip.set_text(text.to_string()).map_err(|e| {
                ToolError::ExecutionFailed(format!("Failed to write to clipboard: {}", e))
            })?;
        ...
        "read" => {
            ...
            let text = clip.get_text().map_err(|e| {
                ToolError::ExecutionFailed(format!("Failed to read clipboard: {}", e))
            })?;
            Ok(serde_json::Value::String(format!(
                "Clipboard contents:\n{}",
                text
        ...
        _ => Err(ToolError::ExecutionFailed(
            "Unknown clipboard action. Use 'read' or 'write'.".to_string(),
        )),
    }
}
...
pub async fn notify(title: String, message: String) -> Result<serde_json::Value, ToolError> {
    #[cfg(target_os = "macos")]
    {
        let script = format!(
        ...
        let output = Command::new("osascript")
            ...
            .output()
            .map_err(|e| {
                ToolError::ExecutionFailed(format!("Failed to execute osascript: {}", e))
            })?;
    ...
    #[cfg(target_os = "linux")]
    {
        let output = Command::new("notify-send")
            ...
            .output();
        match output {
            Ok(o) if o.status.success() => Ok(serde_json::Value::String(format!(
                "🔔 Notification sent: {} — {}",
                title, message
            ))),
            Ok(o) => {
    ...
    #[cfg(not(any(target_os = "macos", target_os = "linux")))]
    {
        ...
        Ok(serde_json::Value::String(
            "Notification not supported on this platform.".to_string(),
        ))
    }
}
...
pub async fn env_var(action: String, name: Option<String>) -> Result<serde_json::Value, ToolError> {
    match action.as_str() {
        "get" => {
            let var_name = name.ok_or_else(|| {
                ToolError::ExecutionFailed("Missing 'name' for env get".to_string())
            })?;
        ...
        _ => Err(ToolError::ExecutionFailed(
            "Unknown env_var action. Use 'get' or 'list'.".to_string(),
        )),
    }
}
...
pub async fn calculator(expression: String) -> Result<serde_json::Value, ToolError> {
