...
impl AgentTool for SearchWebTool {
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<SearchWebArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        ...
        let tr_selector =
            scraper::Selector::parse("tr").map_err(|e| miette!("Selector Error: {:?}", e))?;
        let a_selector = scraper::Selector::parse("a.result-link")
            .map_err(|e| miette!("Selector Error: {:?}", e))?;
        let snippet_selector = scraper::Selector::parse("td.result-snippet")
            .map_err(|e| miette!("Selector Error: {:?}", e))?;
...
impl AgentTool for ReadUrlTool {
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<ReadUrlArgs>();
...
impl AgentTool for HttpRequestTool {
    ...
    fn description(&self) -> &'static str {
        "Makes an arbitrary HTTP request. Use this ONLY as a last resort for debugging REST APIs or webhooks. DO NOT use this for gathering standard web data, stocks, or searches."
    }
    fn tool_info(&self) -> ToolInfo {
        let mut settings = schemars::generate::SchemaSettings::draft07();
        settings.inline_subschemas = true;
        let payload = settings
            .into_generator()
            .into_root_schema_for::<HttpRequestArgs>();
...
impl AgentTool for DownloadFileTool {
    ...
    fn description(&self) -> &'static str {
        "Download a file from a URL and save it to a local path. Useful for fetching remote resources, images, scripts, or data files."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<DownloadFileArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        let typed_args: DownloadFileArgs =
...
pub struct StockScraperArgs {
    #[schemars(
        description = "The stock exchange market identifier code (MIC) (e.g. 'NASDAQ', 'NYSE')"
    )]
    pub exchange: String,
...
impl AgentTool for StockScraperTool {
    ...
    fn description(&self) -> &'static str {
        "CRITICAL TOOL: Fetches real-time stock prices, tickers, and financial data. ALWAYS use this tool when the user asks for the price of a stock (e.g. AAPL, TSLA, MSFT) rather than trying to query a database."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<StockScraperArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        let typed_args: StockScraperArgs =
            serde_json::from_value(args.clone()).into_diagnostic()?;
        ...
        let items_selector = scraper::Selector::parse("div.gyFHrc")
            .map_err(|e| miette!("Selector error: {:?}", e))?;
        let desc_selector = scraper::Selector::parse("div.mfs7Fc")
            .map_err(|e| miette!("Selector error: {:?}", e))?;
        let value_selector = scraper::Selector::parse("div.P6K39c")
            .map_err(|e| miette!("Selector error: {:?}", e))?;
        ...
        if results.is_empty() {
            Ok(format!(
                ...
                typed_args.exchange, typed_args.ticker
            ))
        } else {
            Ok(format!(
