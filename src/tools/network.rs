...
impl AgentTool for NetworkCheckTool {
    ...
    fn description(&self) -> &'static str {
        "Performs low-level ICMP and socket diagnostics. DO NOT USE THIS to verify general internet access. Use ONLY for targeted sysadmin debugging."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<NetworkCheckArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        let typed_args: NetworkCheckArgs =
        ...
        match typed_args.action {
            ...
            NetworkAction::Port => {
                let port = typed_args
                    .port
                    .ok_or_else(|| miette!("Missing 'port' for port check"))?;
                let addr = format!("{}:{}", typed_args.host, port);
                // Simple TCP connect attempt
                match std::net::TcpStream::connect_timeout(
                    &format!("{}:{}", typed_args.host, port)
                        .parse()
                        .unwrap_or_else(|_| {
