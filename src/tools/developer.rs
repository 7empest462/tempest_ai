...
impl AgentTool for InitializeRustProjectTool {
    ...
    fn description(&self) -> &'static str {
        "Creates a new Rust project using 'cargo new' and optionally adds dependencies. A one-turn high-level bootstrap tool."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<InitializeRustProjectArgs>();
    ...
    async fn execute(&self, args: &Value, context: ToolContext) -> Result<String> {
        let typed_args: InitializeRustProjectArgs =
        ...
        let cargo_new_args =
            json!({ "command": cargo_new_cmd, "cwd": path, "timeout_seconds": 60 });
        let res1 = RunCommandTool
            .execute(&cargo_new_args, context.clone())
            .await?;
        ...
        if !deps.is_empty() {
            ...
            let cargo_add_args =
                json!({ "command": cargo_add_cmd, "cwd": project_path, "timeout_seconds": 120 });
            dep_results = RunCommandTool
                .execute(&cargo_add_args, context.clone())
                .await?;
        }
