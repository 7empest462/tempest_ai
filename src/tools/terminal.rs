...
impl AgentTool for TerminalSpawnTool {
    ...
    fn description(&self) -> &'static str {
        "Spawns a new interactive pseudo-terminal (PTY) session. Returns a session_id. Use this to maintain persistent shell sessions, run REPLs, or execute commands that require state to be maintained between calls."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<TerminalSpawnArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        ...
        terminal_registry()
            .lock()
            .map_err(|_| miette!("Terminal registry poisoned"))?
...
impl AgentTool for TerminalInputTool {
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<TerminalInputArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        let typed_args: TerminalInputArgs =
            serde_json::from_value(args.clone()).into_diagnostic()?;
        ...
        let output_buffer = {
            ...
            let session = registry
                .get_mut(id)
                .ok_or_else(|| miette!("Invalid session_id: {}", id))?;
            ...
            session
                .writer
                .write_all(typed_args.input.as_bytes())
                .into_diagnostic()?;
        ...
        let output = {
            let mut buf = output_buffer
                .lock()
                .map_err(|_| miette!("Buffer poisoned"))?;
...
impl AgentTool for TerminalReadTool {
    ...
    fn description(&self) -> &'static str {
        "Reads any accumulated output from a terminal session without sending new input."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<TerminalReadArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        let typed_args: TerminalReadArgs =
            serde_json::from_value(args.clone()).into_diagnostic()?;
        let id = &typed_args.session_id;
...
impl AgentTool for TerminalCloseTool {
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<TerminalCloseArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        let typed_args: TerminalCloseArgs =
            serde_json::from_value(args.clone()).into_diagnostic()?;
