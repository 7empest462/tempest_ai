...
impl AgentTool for GenerateGraphTool {
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings.into_generator().into_root_schema_for::<GenerateGraphArgs>();
    ...
    async fn execute(&self, args: &Value, _context: ToolContext) -> Result<String> {
        ...
        if min_x > max_x || min_y > max_y {
            ...
            min_x = min_x - range_x_pad.max(0.01);
            max_x = max_x + range_x_pad.max(0.01);
            min_y = min_y - range_y_pad.max(0.01);
            max_y = max_y + range_y_pad.max(0.01);
        }

        let output_path = Path::new(&parsed_args.output_path);
        
        let root = BitMapBackend::new(&output_path, (800, 600)).into_drawing_area();
        root.fill(&WHITE).map_err(|e| miette::miette!("Drawing error: {}", e))?;
        ...
        mesh.draw().map_err(|e| miette::miette!("Mesh draw error: {}", e))?;
        ...
        for (i, series) in parsed_args.series.iter().enumerate() {
            if series.x.len() != series.y.len() {
                return Err(miette::miette!("Series '{}' x and y arrays must have the same length", series.name));
            }
            
            let color = *colors[i % colors.len()];
            let data_points: Vec<(f64, f64)> = series.x.iter().copied().zip(series.y.iter().copied()).collect();

            if parsed_args.chart_type.to_lowercase() == "scatter" {
                chart.draw_series(
                    data_points.iter().map(|point| Circle::new(*point, 5, color.filled()))
                )
                .map_err(|e| miette::miette!("Draw error: {}", e))?
                .label(&series.name)
                .legend(move |(x, y)| Circle::new((x, y), 5, color.filled()));
            } else {
                chart.draw_series(LineSeries::new(data_points, color))
                .map_err(|e| miette::miette!("Draw error: {}", e))?
                .label(&series.name)
        ...
        root.present().map_err(|e| miette::miette!("File save error: {}", e))?;

        let abs_path = std::fs::canonicalize(output_path).unwrap_or_else(|_| output_path.to_path_buf());
        Ok(format!("Successfully generated chart and saved to: {}", abs_path.display()))
    }
}
