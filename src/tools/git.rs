...
impl AgentTool for GitStatusTool {
    ...
    fn description(&self) -> &'static str {
        "Lists all changed and untracked files in the current repository."
    }
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<GitStatusArgs>();
...
impl AgentTool for GitDiffTool {
    ...
    fn description(&self) -> &'static str {
        "Shows changes for a specific file or the entire repository."
    }
    fn tool_info(&self) -> ToolInfo {
        let mut settings = schemars::generate::SchemaSettings::draft07();
        settings.inline_subschemas = true;
        let payload = settings
            .into_generator()
            .into_root_schema_for::<GitDiffArgs>();
    ...
    async fn execute(&self, args: &Value, context: ToolContext) -> Result<String> {
        ...
        let path = typed_args.path.unwrap_or_default();
...
impl AgentTool for GitCommitTool {
    ...
    fn description(&self) -> &'static str {
        "Stages and commits changes with a given message."
    }
    ...
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<GitCommitArgs>();
...
impl AgentTool for GitActionTool {
    ...
    fn description(&self) -> &'static str {
        "Natively executes a secure 'git' command. Provide arguments as an array of strings (e.g., ['push', 'origin', 'main'])."
    }
    fn is_modifying(&self) -> bool {
        true
    }
    fn tool_info(&self) -> ToolInfo {
        ...
        let payload = settings
            .into_generator()
            .into_root_schema_for::<GitActionArgs>();
    ...
    async fn execute(&self, json_args: &Value, context: ToolContext) -> Result<String> {
        let typed_args: GitActionArgs =
            serde_json::from_value(json_args.clone()).into_diagnostic()?;
