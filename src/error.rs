...
pub enum FileError {
    ...
    #[diagnostic(
        code(tempest::file::not_found),
        help("Check if the path exists and is correctly typed.")
    )]
    NotFound(String),

    #[error("Permission denied: {0}")]
    #[diagnostic(
        code(tempest::file::permission_denied),
        help("Run with sudo or check file ownership.")
    )]
