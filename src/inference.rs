...
#[cfg(target_os = "macos")]
use mistralrs::{
    EmbeddingRequest, Function, GgufModelBuilder, MemoryGpuConfig, Model,
...
pub struct BackendConfig {
    ...
    pub model: String,
    pub quant: String,
    pub event_tx: Arc<parking_lot::Mutex<Option<tokio::sync::mpsc::Sender<AgentEvent>>>>,
    pub paged_attn: bool,
    pub ctx_limit: usize,
...
pub struct ChatRequest {
    ...
    pub on_tool_call:
        Option<tokio::sync::mpsc::UnboundedSender<ollama_rs::generation::tools::ToolCall>>,
    pub tool_registry: Option<Vec<ollama_rs::generation::tools::ToolInfo>>,
}
...
impl Backend {
    ...
    pub fn raw_history(
        &self,
    ) -> Option<std::sync::Arc<parking_lot::Mutex<Vec<serde_json::Value>>>> {
        match self {
    ...
    pub async fn new(config: BackendConfig) -> Result<(Self, String)> {
        #[cfg(not(target_os = "macos"))]
        {
            let _ = config.quant;
            let _ = config.event_tx;
            let _ = config.paged_attn;
            let _ = config.ctx_limit;
            let _ = config.pa_memory_mb;
        }
        ...
        match config.mode {
            AgentMode::Ollama => {
                ...
                if let Some(remote) = &config.ollama_remote
                    && remote.enabled.unwrap_or(false)
                {
                    let tx_opt = config.event_tx.lock().clone();
                    if let Some(tx) = tx_opt.clone() {
                        let _ = tx
                            .send(AgentEvent::SubagentStatus(Some(format!(
                                "🔒 Establishing SSH Tunnel to {}...",
                                remote.host
                            ))))
                            .await;
                    } else {
                        println!("🔒 Establishing SSH Tunnel to {}...", remote.host);
                    }
                    ...
                    match cmd.spawn() {
                        Ok(mut child) => {
                            ...
                            if let Ok(Some(status)) = child.try_wait() {
                                println!("⚠️ SSH Tunnel exited early with status: {}", status);
                            }
                            register_spawned_process(child);
                        }
                        Err(e) => {
                            println!("❌ Failed to spawn SSH tunnel: {}", e);
                            tokio::time::sleep(tokio::time::Duration::from_millis(1500)).await;
                        }
                    }
            ...
            AgentMode::MLX => {
                #[cfg(target_os = "macos")]
                {
                    ...
                    if let Some(tx) = tx_opt {
                        let _ = tx
                            .send(AgentEvent::SubagentStatus(Some(
                        ...
                        println!(
                            "{} Note: Downloading/Verifying model from Hugging Face if not cached...",
                            "📦".blue()
                        );
                    }
                    ...
                    if let Some(tx) = tx_opt {
                        let _ = tx
                            .send(AgentEvent::SubagentStatus(Some(format!(
                                "⚡ [MLX]: Allocating Metal KV Cache ({}k context)...",
                                config.ctx_limit / 1024
                            ))))
                            .await;
                    } else {
                        if config.paged_attn {
                            println!(
                                "{} [MLX]: Allocating Paged Attention KV Cache (M4 Optimized)...",
                                "⚡".yellow()
                            );
                        } else {
                            println!(
                    ...
                    let (repo, filename_prefix) = if config.model.contains("/") {
                        ...
                        (config.model.clone(), prefix)
                    } else {
                        (config.model.clone(), config.model.clone())
                    };
                    ...
                    let is_gguf = repo.to_lowercase().contains("gguf")
                        ...
                        || std::path::Path::new(&repo).is_dir()
                            && std::fs::read_dir(&repo).is_ok_and(|mut dir| {
                                dir.any(|entry| {
                                    entry.is_ok_and(|e| {
                    ...
                    let mlx_model = if is_gguf {
                        let gguf_file = format!("{}-{}.gguf", filename_prefix, config.quant);
                        ...
                        if config.paged_attn {
                            println!(
                                "{} MLX: Initializing Paged Attention (Window: {} tokens)",
                                "⚡".yellow(),
                                config.ctx_limit
                            );

                            // Determine the memory limit
                            let limit_mb = if let Some(custom_limit) = config.pa_memory_mb {
                                custom_limit
                            } else {
                            ...
                            println!(
                                "{} PagedAttention Budget: {} MB (KV Cache Quantization: F8)",
                                "⚡".yellow(),
                                limit_mb
                            );
                            ...
                            let paged_attn_cfg = PagedAttentionMetaBuilder::default()
                                ...
                                .build()
                                .map_err(|e| {
                                    miette!("Failed to configure Paged Attention: {}", e)
                                })?;
                        ...
                        builder
                            .build()
                            .await
                            .map_err(|e| miette!("Failed to load MLX GGUF model: {}", e))?
                    } else {
                        println!(
                            "{} MLX: Initializing Native Safetensors Backend...",
                            "⚡".yellow()
                        );
                        ...
                        builder
                            .build()
                            .await
                            .map_err(|e| miette!("Failed to load MLX Native model: {}", e))?
                    };
                    ...
                    let mut fallback_client = Ollama::default();
                    if let Some(remote) = &config.ollama_remote
                        && remote.enabled.unwrap_or(false)
                    {
                        let tx_opt = config.event_tx.lock().clone();
            ...
            AgentMode::Bridge => {
                ...
                let provider = crate::ai_bridge::ModelProvider::Ollama { base_url: url };
                ...
                Ok((Backend::Bridge(bridge), config.model))
            }
            AgentMode::LMStudio => {
                let url = config
                    .base_url
            ...
            AgentMode::Kalosm => {
                ...
                if let Some(tx) = tx_opt.clone() {
                    let _ = tx
                        .send(AgentEvent::SubagentStatus(Some(
                            "🚀 Initializing Kalosm Native Backend...".to_string(),
                        )))
                        .await;
                } else {
                ...
                let total_mb = sys.total_memory() / 1024 / 1024;
                if total_mb < 24000
                    && (config.model.contains("Q8")
                        || config.model.contains("q8")
                        || config.model.contains("8_0"))
                {
                    ...
                    if let Some(ref tx) = tx_opt {
                        let _ = tx
                            .send(AgentEvent::SubagentStatus(Some(warning.to_string())))
                            .await;
                ...
                let engine = if config.model == "kalosm_default" || config.model.trim().is_empty() {
                    kalosm::language::Llama::builder()
                        .build()
                        .await
                        .map_err(|e| {
                            miette::miette!("Failed to load default Kalosm model: {}", e)
                        })?
                    ...
                    kalosm::language::Llama::builder()
                        ...
                        .await
                        .map_err(|e| {
                            miette::miette!("Failed to load HuggingFace Kalosm model: {}", e)
                        })?
                } else if std::path::Path::new(&config.model).exists() {
                    let source = kalosm::language::FileSource::Local(std::path::PathBuf::from(
                        &config.model,
                    ));
                    kalosm::language::Llama::builder()
                        .with_source(kalosm::language::LlamaSource::new(source))
                    ...
                    return Err(miette::miette!(
                        "Invalid Kalosm model format: '{}'. Use 'kalosm_default', a local file path, or 'RepoID/Name::filename.gguf' for HF downloads.",
                        config.model
                    ));
                };
            ...
            AgentMode::Gemini => {
                ...
                if let Some(tx) = tx_opt {
                    let _ = tx
                        .send(AgentEvent::SubagentStatus(Some(
                            "🚀 Connecting to Google Gemini API (OpenAI Compat)...".to_string(),
                        )))
                        .await;
                } else {
    ...
    pub async fn stream_chat(&self, request: ChatRequest) -> Result<InferenceOutput> {
        ...
        let options = ModelOptions::default()
            .num_ctx(request.sampling.context_size)
            .num_predict(8192)
        ...
        match self {
            Backend::Kalosm { engine, model: _ } => {
                ...
                if let Some(llama) = engine_lock.as_mut() {
                    ...
                    // Use the model directly for completion since the prompt is already fully formatted
                    let mut stream = llama.complete(&prompt);
                    let mut token_count = 0;
                    let kalosm_max_tokens: usize = request.sampling.context_size as usize / 4; // Allow up to 1/4 of context size

                    while let Some(chunk) = stream.next().await {
                        if request.stop.load(std::sync::atomic::Ordering::Relaxed) {
                            break;
                        }
            ...
            Backend::Bridge(bridge) => {
                ...
                if let Some(tx) = tx_opt {
                    let _ = tx
                        .send(AgentEvent::Thinking(Some("Thinking...".to_string())))
                        .await;
                }

                let mut stream = bridge
                    .stream_chat(history.clone(), request.tool_registry.clone())
                    .await?;
                ...
                if let Some(tx) = tx_opt {
                    let _ = tx
                        .send(AgentEvent::SubagentStatus(Some(
                            "⚡ Bridge: Connection established, waiting for tokens...".to_string(),
                        )))
                        .await;
                }
                ...
                let mut tool_arguments_deltas: std::collections::HashMap<usize, String> =
                        ...
                        if let Some(token) = delta.get("content").and_then(|c| c.as_str()) {
                            ...
                            if first_token {
                                ...
                                if let Some(tx) = tx_opt {
                                    ...
                                    let _ = tx
                                        .send(AgentEvent::SubagentStatus(Some(
                                            "🌊 Stream Active: Receiving tokens...".to_string(),
                                        )))
                                        .await;
                                }
                            }
                                    ...
                                    if let Some(start_idx) = text[current_pos..].find("<think>") {
                                        ...
                                        if !before.trim().is_empty() {
                                            ...
                                            if let Some(tx) = request.event_tx.lock().clone() {
                                                let _ = tx.try_send(AgentEvent::StreamToken(
                                                    before.to_string(),
                                                ));
                                            }
                                        }
                                        is_thinking = true;
                                        if let Some(tx) = request.event_tx.lock().clone() {
                                            let _ = tx.try_send(AgentEvent::ReasoningToken(
                                                "".to_string(),
                                            ));
                                        }
                                        ...
                                        text[current_pos..].find("{\"tool\":")
                                    {
                                        ...
                                        if "</think>".starts_with(pot_tag) {
                                            ...
                                            if !before.is_empty() {
                                                ...
                                                if let Some(tx) = request.event_tx.lock().clone() {
                                                    let _ =
                                                        tx.try_send(AgentEvent::ReasoningToken(
                                                            before.to_string(),
                                                        ));
                                                }
                                            }
                            ...
                            for tc in tool_calls {
                                ...
                                if let Some(func) = tc.get("function") {
                                        ...
                                        func.get("name").and_then(|n| n.as_str())
                                    {
                                        ...
                                        {
                                            if let Ok(num) = stripped[..end].parse::<usize>() {
                                    ...
                                    if !actual_name.is_empty()
                                        && native_tool_calls[extracted_idx].function.name.is_empty()
                                    {
                                    ...
                                    {
                                        let entry =
                                            tool_arguments_deltas.entry(extracted_idx).or_default();
                                        entry.push_str(&args_delta);
                                        if let Some(tx) = request.event_tx.lock().clone() {
                                            let _ =
                                                tx.try_send(AgentEvent::StreamToken(args_delta));
                                        }
                                    }
                                }
                            }
                        ...
                        Err(_e) => {
                            ...
                            match serde_json::from_str::<serde_json::Value>(&repaired) {
                                ...
                                Err(_) => {
                                    ...
                                    if let Some(tx) = request.event_tx.lock().clone() {
                                        let _ = tx.try_send(AgentEvent::SystemUpdate(format!(
                                            "⚠️ Could not parse tool arguments for index {}: {}",
                                            idx,
                                            args_str.chars().take(100).collect::<String>()
                                        )));
                                    }
                                }
                            }
                ...
                // Gemini often sends the first call natively and outputs the rest as text.
                {
                    ...
                    if let Ok(payloads) =
                        llm_extract::extract_all::<ToolCallPayload>(&combined_content)
                        && !payloads.is_empty()
                    {
                                    ...
                                    if let Some(end_idx) = end_idx {
                                        ...
                                        ) && let Some(obj) = val.as_object()
                                        {
                                            ...
                                            let extracted_name = if name_opt.is_none() {
                                                ...
                                                if let Some(tool_comment_idx) =
                                                    prefix.rfind("// Tool:")
                                                {
                                                    let comment_line = &prefix[tool_comment_idx..];
                                                    let name = if let Some(newline_idx) =
                                                        comment_line.find('\n')
                                                    {
                                                        comment_line["// Tool:".len()..newline_idx]
                                                            .trim()
                                                    } else {
                                            ...
                                            if let Some(name) = name_opt {
                                                ...
                                                if let Some(stripped) =
                                                    final_name.strip_prefix("__idx_")
                                                    && let Some(end) = stripped.find("__")
                                                {
                                                    final_name = stripped[end + 2..].to_string();
                                                }
                                                native_tool_calls.push(ollama_rs::generation::tools::ToolCall {
                                                        function: ollama_rs::generation::tools::ToolCallFunction {
                                                            name: final_name,
                ...
                if !native_tool_calls.is_empty()
                    && let Some(pos) = full_content.find('{')
                {
            ...
            Backend::Ollama(ollama, _) => {
                ...
                if let Some(registry) = request.tool_registry {
                    ...
                    if !is_reasoning {
                        request_msg = request_msg.tools(registry);
                    }
                }

                let mut s = None;
                for _attempt in 1..=3 {
                    match ollama.send_chat_messages_stream(request_msg.clone()).await {
                        Ok(res) => {
                            s = Some(res);
                            break;
                        }
                        Err(e) => {
                            ...
                            if let Some(tx) = tx_opt {
                                let _ = tx
                                    .send(AgentEvent::SystemUpdate(format!(
                                        "⚠️ Ollama Error: {}. Retrying (attempt {}/3)...",
                                        e, _attempt
                                    )))
                                    .await;
                            }
                ...
                while let Some(res) = stream.next().await {
                        ...
                        && !thinking.is_empty()
                    {
                        ...
                        if thinking_normalized.len() >= 50 {
                            ...
                            let max_repeats = last_thinking_segments
                                .iter()
                                .filter(|s| *s == &segment)
                                .count();
                            if max_repeats >= 6 {
                                ...
                                if let Some(tx) = request.event_tx.lock().clone() {
                                    let _ = tx.try_send(AgentEvent::SubagentStatus(Some(
                                        "🛑 Thinking Repetition Sentinel triggered".to_string(),
                                    )));
                                }
                                break;
                            }
                    ...
                    while current_pos < text.len() {
                        if !is_thinking && !in_thought_block {
                            ...
                            if first_token && !text.trim().is_empty() {
                                ...
                                if implicit_phrases.iter().any(|&p| lower.starts_with(p)) {
                                    ...
                                    if let Some(tx) = request.event_tx.lock().clone() {
                                        let _ =
                                            tx.try_send(AgentEvent::ReasoningToken("".to_string()));
                                    }
                                }
                            }
                            ...
                            if !in_thought_block {
                                if let Some(start_idx) = text[current_pos..].find("<think>") {
                                    ...
                                    ["THOUGHT:", "PLAN:", "REASONING:", "ANALYSIS:", "[MISSION]"]
                                        .iter()
                                        .filter_map(|&marker| {
                                            if marker == "[MISSION]" {
                                    ...
                                    while end_idx < sub.len()
                                        && (sub.as_bytes()[end_idx] == b':'
                                            || sub.as_bytes()[end_idx] == b'*'
                                            || sub.as_bytes()[end_idx] == b' ')
                                    {
                                        end_idx += 1;
                        ...
                        if !is_thinking && !in_thought_block {
                            ...
                            if !remaining.is_empty() {
                                ...
                                if let Some(tx) = request.event_tx.lock().clone() {
                                    let _ =
                                        tx.try_send(AgentEvent::StreamToken(remaining.to_string()));
                                }
                        ...
                        if is_thinking {
                            if let Some(end_idx) = text[current_pos..].find("</think>") {
                                ...
                                if !got_native_thinking {
                                    ...
                                    if reasoning_content.len() < REASONING_CAP {
                                        ...
                                        reasoning_content.push_str(
                                            &reasoning[..space_left.min(reasoning.len())],
                                        );
                                    }
                                    if let Some(tx) = request.event_tx.lock().clone() {
                                        let _ = tx.try_send(AgentEvent::ReasoningToken(
                                ...
                                // FORGETFUL MODEL RECOVERY: Models forced to use <think>
                            ...
                            if let Some(json_idx) = text[current_pos..].find("```json") {
                                ...
                                current_pos += json_idx; // Don't skip ```json, it belongs to full_content
                            } else if let Some(mission_idx) =
                                text[current_pos..].to_uppercase().find("[/MISSION]")
                            {
                            ...
                            } else if let Some(think_idx) =
                                text[current_pos..].to_uppercase().find("</THINK>")
                            {
                                ...
                                if transitions.iter().any(|&t| lower.starts_with(t)) {
                                    ...
                                    let reasoning =
                                        &text[current_pos..current_pos + newline_idx + 2];
                                    reasoning_content.push_str(reasoning);
                                    if let Some(tx) = request.event_tx.lock().clone() {
                                        let _ = tx.try_send(AgentEvent::ReasoningToken(
                                            reasoning.to_string(),
                    ...
                    if !chunk.message.tool_calls.is_empty() {
                        for call in chunk.message.tool_calls {
                            if let Some(ref tx) = request.on_tool_call {
                    ...
                    if !trimmed.is_empty() && trimmed.len() > 3 {
                        ...
                        if last_segments.len() > 15 {
                            last_segments.remove(0);
                        }
                        if last_segments.iter().filter(|&s| s == trimmed).count() >= 8 {
                            ...
                            let tx = request.event_tx.lock().clone();
                            if let Some(tx) = tx {
            ...
            Backend::MLX {
                ...
                let on_tool_call = request.on_tool_call;
                let tx_opt = request.event_tx.lock().clone();
                if let Some(tx) = tx_opt {
                ...
                if other_messages.is_empty()
                    || other_messages.iter().all(|m| m.role != MessageRole::User)
                {
                    ...
                    if !merged_system.is_empty() {
                        request_builder =
                            request_builder.add_message(TextMessageRole::System, merged_system);
                    }
                    request_builder =
                        request_builder.add_message(TextMessageRole::User, user_content);
                } else {
                    // Normal path
                    if !merged_system.is_empty() {
                        request_builder =
                    ...
                    for msg in other_messages {
                        match msg.role {
                            MessageRole::User => {
                                request_builder =
                                    request_builder.add_message(TextMessageRole::User, msg.content);
                            }
                            MessageRole::Assistant => {
                                if !msg.tool_calls.is_empty() {
                                    ...
                                    request_builder = request_builder
                                        .add_message(TextMessageRole::Assistant, msg.content);
                                }
                            }
                            MessageRole::Tool => {
                                let call_id = format!("call_{}", total_tool_results);
                                request_builder =
                                    request_builder.add_tool_message(msg.content, call_id);
                        ...
                        let mistral_tool = Tool {
                            ...
                            function: Function {
                                ...
                                parameters: {
                                    ...
                                    let schema_val =
                                        serde_json::to_value(&tool.function.parameters)
                                            .unwrap_or(serde_json::json!({}));
                                    if let Some(props) =
                                        schema_val.get("properties").and_then(|p| p.as_object())
                                    {
                ...
                sampling_params.temperature = Some(request.sampling.temperature.into());
                sampling_params.top_p = Some(request.sampling.top_p.into());
                sampling_params.top_k = Some(40);
                sampling_params.repetition_penalty = Some(request.sampling.repeat_penalty);
                ...
                if let Some(tx) = tx_opt.as_ref() {
                    let _ = tx.try_send(AgentEvent::SubagentStatus(Some(
                        "⚡ MLX Engine: Dispatching Request to Metal...".to_string(),
                    )));
                }

                let mut stream = mistral_model
                ...
                if let Some(tx) = tx_opt.as_ref() {
                    let _ = tx.try_send(AgentEvent::SubagentStatus(Some(
                        "⚡ MLX Engine: Stream established, waiting for first token...".to_string(),
                    )));
                }
                ...
                if is_thinking && let Some(tx) = request.event_tx.lock().clone() {
                ...
                while let Some(response) = stream.next().await {
                    ...
                    match response {
                                ...
                                && !reasoning.is_empty()
                            {
                                ...
                                if thinking_normalized.len() >= 50 {
                                    ...
                                    if max_repeats >= 6 {
                                        ...
                                        if let Some(tx) = request.event_tx.lock().clone() {
                                            let _ = tx.try_send(AgentEvent::SubagentStatus(Some(
                                                "🛑 Thinking Repetition Sentinel triggered"
                                                    .to_string(),
                                            )));
                                        }
                                        break;
                            ...
                            if let Some(content) = &chunk.choices[0].delta.content {
                                ...
                                if first_token && !text.trim().is_empty() {
                                    ...
                                    if text.len() >= 10 || !text.starts_with("<") {
                                        ...
                                        if !text.contains("<think>")
                                            && let Some(tx) = request.event_tx.lock().clone()
                                        {
                                            let _ = tx.try_send(AgentEvent::Thinking(None));
                                            let _ = tx.try_send(AgentEvent::SubagentStatus(None));
                                        }
                                ...
                                while current_pos < text.len() {
                                    if !is_thinking && !in_thought_block {
                                        ...
                                        {
                                            let before =
                                                &text[current_pos..current_pos + start_idx];
                                            if !before.is_empty() {
                                                full_content.push_str(before);
                                                if let Some(tx) = request.event_tx.lock().clone() {
                                                    let _ = tx.try_send(AgentEvent::StreamToken(
                                            ...
                                            if !before.is_empty() {
                                                ...
                                                .iter()
                                                .any(|&filler| {
                                                    lower.starts_with(filler) || lower == filler
                                                });
                                                ...
                                                if is_implicit_thinking {
                                                    ...
                                                    {
                                                        let _ =
                                                            tx.try_send(AgentEvent::StreamToken(
                                                                before.to_string(),
                                                            ));
                                                    }
                                                    full_content.push_str(before);
                                            ...
                                            if !is_implicit_thinking {
                                                ...
                                                if let Some(tx) = request.event_tx.lock().clone() {
                                                    let _ = tx.try_send(
                                                        AgentEvent::ReasoningToken("".to_string()),
                                                    );
                                                }
                                            }
                                            ...
                                            let upper = text[current_pos..].to_uppercase();
                                            let markers =
                                                ["THOUGHT:", "PLAN:", "REASONING:", "ANALYSIS:"];
                                            ...
                                            for marker in markers {
                                                for i in 1..marker.len() {
                                                    ...
                                                    if upper.ends_with(partial) {
                                                        let pos = upper.len() - i;
                                                        if best_partial.is_none_or(|(p, _)| pos < p)
                                                        {
                                            ...
                                            let before =
                                                &text[current_pos..current_pos + found_idx_rel];
                                            if !before.is_empty() {
                                                full_content.push_str(before);
                                                if let Some(tx) = request.event_tx.lock().clone() {
                                                    let _ = tx.try_send(AgentEvent::StreamToken(
                                            ...
                                            tag_residue =
                                                text[current_pos + found_idx_rel..].to_string();
                                            break;
                                        } else {
                                            let remaining = &text[current_pos..];
                                            full_content.push_str(remaining);
                                        ...
                                        {
                                            let reasoning =
                                                &text[current_pos..current_pos + end_idx];
                                            reasoning_content.push_str(reasoning);
                                            if let Some(tx) = request.event_tx.lock().clone() {
                                                let _ = tx.try_send(AgentEvent::ReasoningToken(
                                                    reasoning.to_string(),
                                            ...
                                            if first_token {
                                                ...
                                                if let Some(tx) = request.event_tx.lock().clone() {
                                                    let _ = tx.try_send(AgentEvent::Thinking(None));
                                                    let _ = tx
                                                        .try_send(AgentEvent::SubagentStatus(None));
                                                }
                                            }
                                        } else if let Some(json_idx) =
                                            text[current_pos..].find("```json")
                                        {
                                            // FAILSAFE: Implicit end of explicit thinking block
                                            let reasoning =
                                                &text[current_pos..current_pos + json_idx];
                                            reasoning_content.push_str(reasoning);
                                            if let Some(tx) = request.event_tx.lock().clone() {
                                                let _ = tx.try_send(AgentEvent::ReasoningToken(
                                                    reasoning.to_string(),
                                        ...
                                        } else if let Some(last_lt) = text[current_pos..].rfind('<')
                                        {
                                            ...
                                            if "</think>".starts_with(pot_tag) {
                                                let before =
                                                    &text[current_pos..current_pos + last_lt];
                                                if !before.is_empty() {
                                                    reasoning_content.push_str(before);
                                                    if let Some(tx) =
                                                        request.event_tx.lock().clone()
                                        ...
                                        {
                                            let reasoning =
                                                &text[current_pos..current_pos + mission_idx];
                                            reasoning_content.push_str(reasoning);
                                            if let Some(tx) = request.event_tx.lock().clone() {
                                                let _ = tx.try_send(AgentEvent::ReasoningToken(
                                                    reasoning.to_string(),
                                            ...
                                            in_thought_block = false;
                                            current_pos += mission_idx + 10;
                                        } else if let Some(done_idx) =
                                            text[current_pos..].find("DONE:")
                                        {
                                            let reasoning =
                                                &text[current_pos..current_pos + done_idx];
                                            reasoning_content.push_str(reasoning);
                                            if let Some(tx) = request.event_tx.lock().clone() {
                                                let _ = tx.try_send(AgentEvent::ReasoningToken(
                                                    reasoning.to_string(),
                                            ...
                                            text[current_pos..].find("\n\n")
                                        {
                                            ...
                                            if transitions.iter().any(|&t| lower.starts_with(t)) {
                                                let reasoning =
                                                    &text[current_pos..current_pos + newline_idx];
                                                reasoning_content.push_str(reasoning);
                                                if let Some(tx) = request.event_tx.lock().clone() {
                                                    let _ =
                                                        tx.try_send(AgentEvent::ReasoningToken(
                                                ...
                                                let reasoning = &text
                                                    [current_pos..current_pos + newline_idx + 2];
                                                reasoning_content.push_str(reasoning);
                                                if let Some(tx) = request.event_tx.lock().clone() {
                                                    let _ =
                                                        tx.try_send(AgentEvent::ReasoningToken(
                                            ...
                                            for i in 1..=7 {
                                                if remaining.ends_with(&"```json"[..i]) {
                                                    save_residue = i;
                                                }
                                            }
                                            // Check for partial DONE:
                                            for i in 1..=5 {
                                                if remaining.ends_with(&"DONE:"[..i]) {
                                                    save_residue = i;
                                                }
                                            }
                                ...
                                if !trimmed.is_empty() {
                                    ...
                                    if trimmed.len() > 3 {
                                        ...
                                        if last_segments.iter().filter(|&s| s == trimmed).count()
                                            >= 8
                                        {
                                            let warning = "\n\n⚠️ [REPETITION SENTINEL]: Breaking loop to prevent hallucination plateau.";
                                            full_content.push_str(warning);
                                    ...
                                    // 🛡️ BLOCK-LEVEL REPETITION GUARD (Checks accumulated full_content for line-repetition)
                                    {
                                        ...
                                        for line in full_content.lines() {
                                            ...
                                            if line_trimmed.len() >= 15 {
                                                let count =
                                                    line_counts.entry(line_trimmed).or_insert(0);
                                                *count += 1;
                                                if *count >= 6 {
                                                    loop_detected = true;
                                                    break;
                            ...
                            if crate::overwatch::is_complete_tool_json(&full_content) {
                                ...
                                if let Some(tx) = tx {
                                    let _ = tx.try_send(AgentEvent::SystemUpdate(
                                        "🛑 Hard Stop: Tool call detected, yielding execution."
                                            .to_string(),
                                    ));
                                }
                                break;
                        ...
                        MistralResponse::ModelError(e, _) => {
                            return Err(miette!("MLX Model Error: {}", e));
                        }
                        _ => {}
                    }
                }
                ...
                && !payloads.is_empty()
            {
                ...
                for payload in payloads {
                    ...
                    if final_name.starts_with("__idx_")
                        && let Some(end) = final_name[6..].find("__")
                    {
        ...
        match self {
            ...
            Backend::MLX {
                ...
                if let Some(embed) = embedder {
                    let request =
                        EmbeddingRequest::builder().add_prompt(format!("passage: {}", text));
                    let res = embed
            ...
            Backend::Bridge(bridge) => bridge.generate_embeddings(text.to_string()).await,
    ...
    for msg in normalized {
            ...
            && last.role == msg.role
        {
            ...
            for tc in msg.tool_calls {
                if !last.tool_calls.iter().any(|existing| {
...
pub fn kill_all_spawned_processes() {
    if let Some(mutex) = SPAWNED_PROCESSES.get() {
