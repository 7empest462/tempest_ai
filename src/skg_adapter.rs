...
impl Provider for SkgBackendProvider {
    fn infer(&self, request: InferRequest) -> impl Future<Output = Result<InferResponse, ProviderError>> + Send {
        ...
        async move {
            ...
            let output = backend.stream_chat(
                model.clone(),
                history,
                sampling,
                event_tx,
                ...
                None,
                None,
            ).await.map_err(|e| ProviderError::Other(e.into()))?;
            ...
            Ok(InferResponse {
                ...
                stop_reason: if tool_calls.is_empty() { skg_turn::types::StopReason::EndTurn } else { skg_turn::types::StopReason::ToolUse },
