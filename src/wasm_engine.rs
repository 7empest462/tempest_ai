...
impl WasmSandboxEngine {
    ...
    pub fn test_infinite_loop(&self) -> Result<()> {
        ...
        let module = Module::new(&self.engine, wat)
            .map_err(|e| miette!("Failed to compile loop WAT: {:?}", e))?;
        ...
        if res.is_err() {
            ...
            Err(miette!(
                "Expected infinite loop to be interrupted by fuel limit, but it finished successfully!"
            ))
        }
    }
}
...
mod tests {
    ...
    fn test_wasm_infinite_loop_preemption() {
        ...
        assert!(
            loop_res.is_ok(),
            "Infinite loop was not successfully interrupted by the fuel cap!"
        );
    }
}
