...
use crate::mcp_protocol::{ChatPayload, JsonRpcRequest, TempestRequest, TempestResponse};
...
impl McpServer {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        tokio::spawn(async move {
            while let Some(event) = rx.recv().await {
                match event {
                    ...
                    crate::tui::AgentEvent::SubagentStatus(Some(text)) => {
                        let envelope = json!({ "jsonrpc": "2.0", "method": "tempest/status", "params": { "text": text } });
                        let _ = stdout_clone
                            .write_all(
                                (serde_json::to_string(&envelope).unwrap() + "\n").as_bytes(),
        ...
        loop {
            ...
            if method.starts_with("tempest/") {
                ...
                self.handle_tempest_request(id, tempest_req.payload, &mut stdout)
                    .await?;
                continue;
            }
            ...
            let response = match method {
                ...
                "tools/call" => {
                    let params = request
                        .get("params")
                        .ok_or_else(|| miette!("Missing params"))?;
                    let name = params
                        .get("name")
                        .and_then(|n| n.as_str())
                        .ok_or_else(|| miette!("Missing tool name"))?;
                    let arguments = params.get("arguments").cloned().unwrap_or(json!({}));

                    let result = self.agent.execute_tool_by_name(name, &arguments).await;
                    match result {
                        Ok(output) => {
                            json!({ "jsonrpc": "2.0", "id": id, "result": { "content": [{"type": "text", "text": output}] } })
                        }
                        Err(e) => {
    ...
    async fn handle_tempest_request(
        &self,
        id: Option<Value>,
        req: TempestRequest,
        stdout_param: &mut tokio::io::Stdout,
    ) -> Result<()> {
        match req {
            TempestRequest::Chat {
                message,
                ...
                tokio::spawn(async move {
                    ...
                    *active_chat_id_clone.lock() = None;
                    ...
                    if let Some(ref current_flag) = *flag_lock
                        && std::sync::Arc::ptr_eq(current_flag, &stop_flag_val)
                    {
                        *flag_lock = None;
                    }
                });
