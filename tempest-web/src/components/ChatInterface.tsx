...
export function ChatInterface() {
  const { messages, isStreaming, addMessage, streamAccumulator, reasoningAccumulator, currentToolResults, activeFile } = useStore();
  ...
  return (
    <div className="flex flex-col h-full w-full">
      ...
      >
          ...
          {messages.map((msg) => (
            <motion.div
              ...
              className={`max-w-[85%] p-4 rounded-xl text-sm leading-relaxed ${
                msg.role === 'system' ? 'bg-accent/10 border border-accent/20 font-mono self-start' :
                msg.role === 'ai' ? 'bg-white/5 border border-white/10 self-start' :
            ...
            >
              ...
              {msg.reasoning && (
                <details className="text-xs text-muted-foreground border-l-2 border-accent/50 pl-2 mb-2">
                  <summary className="cursor-pointer font-semibold select-none hover:text-white transition-colors">Thought Process</summary>
                  <div className="mt-1 font-mono whitespace-pre-wrap opacity-70">{msg.reasoning}</div>
                </details>
              )}
                  ...
                  {msg.tools.map((t, idx) => (
                    <details key={idx} className="bg-black/20 border border-white/5 rounded block">
                      ...
                      <div className="p-2 border-t border-white/5 text-[10px] font-mono">
                        {t.args && <div className="mb-1"><strong className="text-muted-foreground">Input:</strong><pre className="whitespace-pre-wrap text-white/70 overflow-x-auto">{t.args}</pre></div>}
                        {t.output && <div><strong className="text-muted-foreground">Output:</strong><pre className="whitespace-pre-wrap text-white/70 overflow-x-auto max-h-40">{t.output}</pre></div>}
                      </div>
                    </details>
                  ))}
                </div>
        ...
        {isStreaming && (reasoningAccumulator || streamAccumulator || currentToolResults.length > 0) && (
          <div className="max-w-[85%] p-4 rounded-xl text-sm leading-relaxed bg-white/5 border border-white/10 self-start flex flex-col gap-2">
            {reasoningAccumulator && (
              <details open className="text-xs text-muted-foreground border-l-2 border-accent/50 pl-2 mb-1">
                <summary className="cursor-pointer font-semibold select-none hover:text-white transition-colors">Thinking Process</summary>
                <div className="mt-1 font-mono whitespace-pre-wrap opacity-70">{reasoningAccumulator}</div>
              </details>
            )}
            {currentToolResults.length > 0 && (
              <div className="flex flex-col gap-2 mb-1">
                {currentToolResults.map((t, idx) => (
                  <details key={idx} open className="bg-black/20 border border-white/5 rounded block">
                    <summary className="text-xs cursor-pointer select-none py-1 px-2 text-purple-400 font-semibold hover:bg-white/5 transition-colors">
                      🔧 Tool: {t.name} {t.success ? '✅' : '❌'}
                    </summary>
                    <div className="p-2 border-t border-white/5 text-[10px] font-mono">
            ...
            {streamAccumulator && (
              <div className="whitespace-pre-wrap">{streamAccumulator}</div>
            )}
          </div>
        )}
      </div>
