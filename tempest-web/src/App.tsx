...
export default function App() {
  ...
  const { 
    isConnected, 
    engineStatus,
    cpu, gpu, ram, tps, ctxUsed, ctxTotal,
    backgroundIntensity,
    isEditorFocused,
    activeFile,
    setActiveFile,
  ...
  const bgOpacity = backgroundIntensity === 'subtle' ? 'opacity-20' : 
                    backgroundIntensity === 'medium' ? 'opacity-50' : 'opacity-100';
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex flex-col relative text-foreground">
      ...
      <header className="flex-none h-14 glass-panel border-b border-border/50 flex items-center justify-between px-6 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold tracking-widest">
            TEMPEST <span className="text-accent drop-shadow-[0_0_8px_rgba(0,242,255,0.4)]">AI</span>
          </h1>
        </div>
        <div className="flex items-center gap-6">
          ...
          <div className="flex gap-4 font-mono text-xs text-muted-foreground">
            <span className="bg-white/5 px-3 py-1 rounded-md border-l-2 border-accent">CPU: {cpu.toFixed(1)}%</span>
            <span className="bg-white/5 px-3 py-1 rounded-md border-l-2 border-accent">GPU: {gpu.toFixed(1)}%</span>
            <span className="bg-white/5 px-3 py-1 rounded-md border-l-2 border-accent">RAM: {ram}</span>
          </div>
        </div>
      </header>
                      ...
                      onClick={() => {
                        if (isFileEditable) {
                          ...
                          if (window.sendNexus && activeFile) {
                            // @ts-ignore
                            window.sendNexus('WriteFile', { path: activeFile.name, content: activeFile.content });
                          }
                        }
      ...
      <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 w-[80%] h-64 glass-panel border border-border/50 rounded-t-xl z-30 transition-all duration-300 ${isTerminalOpen ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0 pointer-events-none'}`}>
        <div className="flex items-center justify-between p-2 border-b border-border/50 bg-black/40">
          ...
          <button onClick={() => setTerminalOpen(false)} className="text-muted-foreground hover:text-white px-2">✕</button>
        </div>
      ...
      <footer className="flex-none h-10 glass-panel border-t border-border/50 flex items-center justify-between px-6 z-10 text-xs font-mono text-muted-foreground mt-4">
        <div className="flex items-center gap-3">
          <span className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-500 shadow-[0_0_8px_#00ff88]' : 'bg-red-500 shadow-[0_0_8px_#ff0000]'}`} />
          <span>Engine: {engineStatus}</span>
        </div>
        <div className="flex items-center gap-4">
          <span>TPS: {tps}</span>
          <span>│</span>
          <span>CTX: {ctxUsed >= 1024 ? `${(ctxUsed / 1024).toFixed(1)}k` : ctxUsed}/{ctxTotal >= 1024 ? `${(ctxTotal / 1024).toFixed(0)}k` : ctxTotal}</span>
        </div>
      </footer>
    </div>
  );
